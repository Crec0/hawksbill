...
import dev.crec.hawksbill.api.util.SPACE_1EM
import dev.crec.hawksbill.api.util.hashString
import dev.crec.hawksbill.api.util.truncate
import dev.crec.hawksbill.module.database.Reminder
...
class CommandRemindMe : ICommand {
    ...
    init {
        Timer().schedule(
            object : TimerTask() {
                override fun run() {
                    val collection = HawksBill.database.getCollection<Reminder>()
                    ...
                    if (expiredReminders.isNotEmpty()) {
                        ...
                        collection.deleteMany(
                            Reminder::reminder_id `in` expiredReminders.map { it.reminder_id }
                        )
                    }
                }
            },
    ...
    private fun handleAddReminder(event: SlashCommandInteractionEvent) {
        ...
        event.deferReply()
            ...
            .queue {
                HawksBill.database.getCollection<Reminder>().insertOne(
                    Reminder(
                        reminder_id = hash,
                        member_id = user.id,
    ...
    private fun handleCancelReminder(event: SlashCommandInteractionEvent) {
        ...
        val reminder = HawksBill.database.getCollection<Reminder>().findOne(Reminder::reminder_id eq reminderId)

        val content = when (reminder?.member_id) {
            null -> "No reminder found with id: $reminderId"
            event.user.id -> {
                HawksBill.database.getCollection<Reminder>().deleteOne(Reminder::reminder_id eq reminderId)
    ...
    private fun handleListReminders(event: SlashCommandInteractionEvent) {
        ...
        val reminders = HawksBill.database.getCollection<Reminder>().find(Reminder::member_id eq user.id).toList()
    ...
    private fun sendExpiredMessage(reminder: Reminder) {
        val channel = HawksBill.jda.getChannel<TextChannel>(reminder.channel_id)
        val user = HawksBill.jda.retrieveUserById(reminder.member_id).complete()
